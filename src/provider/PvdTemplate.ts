    ...
    public static process(e) {
        ...
        if (templates != null) {
            if (templates.length > 0) {
                ...
                vscode.window.showWarningMessage("No template files in " + ExtMgr.templateDir + ", please change 'luaide-lite-up.templateDir'")
            }
        }
    }
    ...
    private static showInputBox(selection, e, prompt) {
        ...
        vscode.window.showInputBox(inputOptions).then(function (filename) {
            if (filename == "") {
                ...
                if (!isCopyright) {
                    content = content.replace(new RegExp("{copyright}", "gm"), "2018 luaide-lite-up")
                }
                if (!isAuthor) {
                    content = content.replace(new RegExp("{author}", "gm"), "luaide-lite-up")
                }
